...
do {                                                                        \
    ...
    if (_de_off > MAX_VAL_SIZE - sizeof(u64)) break;                        \
    unsigned int _de_sz = (param)->derefs[J].size;                          \
    if (_de_sz > DEREF_ENTRY_SIZE) _de_sz = DEREF_ENTRY_SIZE;              \
    u64 _de_ptr = 0;                                                        \
    bpf_probe_read_kernel(&_de_ptr, sizeof(u64), val_start + _de_off);     \
    char *_de_dst = (scratch) + sizeof(param_event_t) + MAX_VAL_SIZE        \
                    + (J) * DEREF_ENTRY_SIZE;                               \
    /* Skip nil pointers: the Go-side rewritePointersToFakeAddresses       \
     * already handles nil by checking the pointer value in the val         \
     * region, so there is nothing useful to write here.                    \
     * We avoid __builtin_memset with runtime size (_de_sz) because the    \
     * BPF backend does not support memset with non-constant size. */       \
    if (_de_ptr != 0)                                                       \
        bpf_probe_read_user(_de_dst, _de_sz, (void *)_de_ptr);             \
} while (0)
...
do {                                                                        \
    ...
    bpf_ringbuf_output(&events, scratch, _evt_sz, BPF_RB_FORCE_WAKEUP);  \
} while (0)
